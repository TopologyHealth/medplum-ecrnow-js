...
async function buildContext(resourceType: ResourceType, resourceId: string): Promise<Context> {
...
async function performAction(pdToProcessUrl: string, actionId: string, reportEndpoint: string, context: Context) {
  if (process.env.MEDPLUM_CLIENT_ID === undefined) throw new Error("MEDPLUM_CLIENT_ID environment variable is missing");
  if (process.env.MEDPLUM_CLIENT_SECRET === undefined) throw new Error("MEDPLUM_CLIENT_SECRET environment variable is missing");
  ...
  switch (actionCode) {
    ...
    case "submit-report": {
      ...
      console.dir(await fetch(reportEndpoint, { method: "POST", body: JSON.stringify(reportToSubmit), headers: { 'Content-Type': 'application/fhir+json' } }), {depth: undefined});
      break;
    }
...
app.post(`/${ENDPOINT_PATH}`, async (req: express.Request<any, any, Resource>, res: express.Response, next) => {
  try {
    ...
    return await performAction(pdToProcess, actionToProcess, reportEndpoint, context);
  } catch (error) {
...
try {
  ...
  try {
    ...
    console.log(await performAction("http://hl7.org/fhir/us/central-cancer-registry-reporting/StructureDefinition/plandefinition-central-cancer-registry-reporting-example", "start-workflow", "https://long-dream-6bf8.michael-jackson-222.workers.dev", context));
